...
{
  uintptr_t highest_va = mem_base - first_free_paddr;
  ...
  for (uintptr_t vaddr = info.first_user_vaddr, paddr = vaddr + info.load_offset, end = info.first_vaddr_after_user;
       paddr < mem_base + mem_size; vaddr += MEGAPAGE_SIZE, paddr += MEGAPAGE_SIZE) {
  ...
  uintptr_t num_sbi_pages = ((uintptr_t)&_sbi_end - mem_base - 1) / RISCV_PGSIZE + 1;
  assert(num_sbi_pages <= (1 << RISCV_PGLEVEL_BITS));
  for (uintptr_t i = 0; i < num_sbi_pages; i++) {
    uintptr_t idx = (1 << RISCV_PGLEVEL_BITS) - num_sbi_pages + i;
    sbi_pt[idx] = pte_create((mem_base / RISCV_PGSIZE) + i, PTE_G | PTE_R | PTE_X);
  }
